...
router.get('/', function (req, res, next) {
  ...
  res.render('index', { title: 'Express 점심11:50', name:req.session.name });
});
...
// get방식, post방식 등 method방식과 혼용해서 사용가능
// ~/url/:변수명

router.get('/info/:no', (req, res, next) => {
  //u값을 기준으로 존재하지 않으면 미리보기 화면, 값이 존재하면 수정화면으로 설정하겠다
  ...
  //no를 넣어서 해당 상품 정보를 가져와서(sql) 화면에 뿌린다(render)
  //함수명 : selectItembyNo
