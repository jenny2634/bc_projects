/*
    ...
    -> cherrio 모듈은 html에서 데이터를 추출하는 모듈 -> 데이터 구축 : 추출 => 디비에 적재
    $ npm i --save cheerio
    $ npm i --save request
*/
...
request( target_site, (err, response, body)=>{
    if(!err){
        ...
        //1.body의 내용을 DOM으로 생성한다  -> 형식은 jQuery 방식
        let $ = cheerio.load( body ) 
        console.log(typeof $)
        //2.추출 -> css selector를 통해서 대상을 특정하고, 추출한다
        //2-1. 카테고리만 뽑아보자 : 특정한 내용은, class가 mdl-navigation__link만 요소
        //     요소(Element):시작태그 + 콘텐츠 + 종료태그 => <a>하이</a>
        //     class의 css selector는 .class값
        //console.log( $('.mdl-navigation__link').text() )
        console.log( $('.mdl-navigation__link')['1'] )
       //               id => #, 요소명 : 문서상에 존재하는 모든 요소명, 

       //상품 목록 특징 : mdl-cell mdl-card mdl-shadow--4dp portfolio-card
