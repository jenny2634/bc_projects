...
contract BasicArrayTest
{
    ...
    {
        //memory변수는 명시적으로 memory키워드를 사용해야 한다
        //함수 호출시 내부에만 존재하는 임시 변수(오직 함수내부)
        //storage변수 대비 저렴하다(싸다, 비용이 들지 않는다)
        uint8[3] memory arr1 = [1,2,3];
        uint8[] memory arr2 = new uint8[](arr1.length);
        //상태변수를 지역변수에 대입하는 행위 => copy
        //상태변수를 memory 변수에 넣고  memory 를 수정하면, 상태변수에 영향을 미치지 않는다
        //왜? 상태변수값이 카피되기 때문에 상태변수는 보전된다
        uint256[] memory arr5 = myArr4;
        arr5[0] = 10;
        return arr5[0];
    }
    
    function arrTset1_sub() public view returns (uint256)
    {
        return myArr4[0];
    }
    ...
    {
        //bytes : 정적 바이트 배열, 선언시 고정 크기를 가진다
        arr;
        arr="AB";
        //사이즈 overfull -> error
        //arr="ABC";
        //바이트 배열을 동적으로 구성하여, 사이즈 넘었을 때 사용하면 
        //유동적으로 조절이 된다. -> gas발생, 고정크기를 권장
