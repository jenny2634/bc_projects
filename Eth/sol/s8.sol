...
contract BasicString
{
    //문자열 기초 라이브러리 만들기
    //byte[] -> string
     function byteArrayToString( byte[] memory src)
     public pure returns(string memory)
     { 
        //byte[] -> bytes -> string
        //byte[] -> bytes
        //입력한 byte배열의 크기만큼 bytes의 크기를 잡아준다
        bytes memory buffer = new bytes(src.length);
        //인덱스별로 카피
        for(uint8 i=0;i<src.length;i++){
            buffer[i] = src[i];
        }
        //문자열 변환, 모든 데이터는bytes에 담겼으므로, string 변환
        return string(buffer);
     }
    //string -> bytes -> byte[] -> bytes -> string
    //string -> bytes -> byte[] 함수
    function stringToByteArray(string memory src)
    public pure returns ( byte[] memory)
    {
        //string -> bytes
    ...
    //test
    //string -> bytes -> byte[] -> bytes -> string
    function testSTBATS(string memory src)
    public pure returns (string memory)
    {
    ...
    //문자열화 시키면 완성
    //1.함수의 외형부터 완성
    function addstring(string memory a, string memory b) 
    public pure returns (string memory)
    {
        //2. input string -> bytes로 변환 2개, s1,s2 variable name
    ...
    //output: string
    function subString(string memory src, uint8 offset, uint8 size) 
    public pure returns (string memory)
    {
        //string -> bytes
        bytes memory tmp = bytes(src);
